...
def valid_prompt(prompt, model_id):
    try:
        ...
        print("Prompt category:", category)
...
def query_knowledge_base(query, kb_id):
    """
    تستدعي Bedrock Knowledge Base وتعيد قائمة retrievalResults كما هي.
    هذا الشكل متوافق مع app.py الحالي، اللي يبني الـ context بهالشكل:
...
def generate_response(prompt, model_id, temperature, top_p, context=None):

    try:
        if context:
            full_prompt = f"""
Human: You are a helpful assistant that answers questions ONLY using the provided context.
